...
def greedy_algo(V_ksparse,
    ...
    for j in range(number_node_sampled):
        if fast_version:  # Used the approximation to reduce the time complexity
            ...
            K = update_K(K, W, cov_w, u,
                         get_v)  # not suppose to do that after all
        else:
            u = argmax(V_ksparse, V_ksparse_H, get_v, H, H_h, cov_x, cov_w,
...
def random_leverage_algo(V_ksparse, number_node_sampled):
    ...
    list_random_leverage = np.random.choice(
        choices, number_node_sampled, replace=False, p=normalized_norms)
...
def uniform_random_algo(number_node_sampled, num_nodes):
    ...
    return [
        int(i)
        for i in np.random.choice(choices, number_node_sampled, replace=False)
    ]


def brute_force_algo(V_ksparse, V_ksparse_H, get_v, H, H_h, cov_x, cov_w, W,
                     number_node_sampled, num_nodes):
    subset_scores = {}
    optimal_subset = []
    optimal_K_T = get_upper_bound_trace_K(W, cov_x)
    c = False
    all_possible_set_combination = combinations(
        range(num_nodes), number_node_sampled)
    for possible_set in all_possible_set_combination:  # Try every subset
        score = get_MSE_score(V_ksparse, V_ksparse_H, get_v, H, H_h, cov_x,
                              cov_w, possible_set)
    ...
    empty_set_score = get_MSE_score(V_ksparse, V_ksparse_H, get_v, H, H_h,
                                    cov_x, cov_w, [])
