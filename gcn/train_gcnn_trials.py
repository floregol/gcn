...
def train_results(trials, sampler, label_percent, model_gcn, adj, features, y_train, y_val, y_test,
    ...
    for trial in range(trials):
        train_mask, label_percent = sampler.get_train_mask_fun()
...
# def train_results(trials, sampler, label_percent, model_gcn,
#                   get_train_mask_fun, adj, features, y_train, y_val, y_test,
#                   initial_train_mask, val_mask, test_mask,
#                   MAINTAIN_LABEL_BALANCE, WITH_TEST,
#                   SHOW_TEST_VAL_DATASET_STATS, VERBOSE_TRAINING, settings,
#                   fileinfo, stats_adj_helper):
#     result = []
#     acc_trials = np.zeros((trials, ))
...
#         pool.apply_async(
#             trial_run,
#             (num_iter, label_percent, model_gcn, sampler, adj,
...
#              stats_adj_helper)) for indices in range(cores)
#     ]
#     pool.close()
#     pool.join()
...
# def trial_run(num_iter, label_percent, model_gcn, get_train_mask_fun, adj,
#               features, y_train, y_val, y_test, initial_train_mask, val_mask,
#               test_mask, MAINTAIN_LABEL_BALANCE, WITH_TEST,
#               SHOW_TEST_VAL_DATASET_STATS, VERBOSE_TRAINING, settings,
#               fileinfo, stats_adj_helper):
