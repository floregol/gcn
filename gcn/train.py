...
def train_model(model_func,
    ...
    for epoch in range(FLAGS.epochs):
        ...
        # Training step
        outs = sess.run(
            [model.opt_op, model.loss, model.accuracy], feed_dict=feed_dict)
        # Validation
        ...
        if VERBOSE_TRAINING:
            ...
            print("Epoch:", '%04d' % (epoch + 1), "train_loss=",
                  "{:.5f}".format(outs[1]), "train_acc=", "{:.5f}".format(
                      outs[2]), "val_loss=", "{:.5f}".format(cost), "val_acc=",
                  "{:.5f}".format(acc), "time=",
    ...
    labels_equal = (np.equal(
        np.argmax(predicted_labels, axis=1), np.argmax(y_test, axis=1)))
